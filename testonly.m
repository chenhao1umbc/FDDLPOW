...
r_zf =  zeros(3,3,5,5); % 3 algs; L=1,2,3; 5 dynamic ratio; 5 folds; r_mf = r_knn;
...
mixture_n =1; 
r_knn = zeros(3, 5, 5); % 3 algs; SNR= 20 , 0, -10, -20, -30; 5 folds
...
for alg = 1:3       
    ...
    alg_nm = ['opts.Dict',num2str(alg), 'nm'];
    load(alg_nm,'Dict','opts')
    disp(alg_nm)
    ...
    % KNN classifier
    acc_knn = myknn(Xtr, Xtest, Database, cvortest); %
    r_knn(alg, ind_snr ,f) = acc_knn; 
end
end   
end
