...
%         Z = sparsecoding(Dict, Database, opts, mixture_n, cvortest); % regular sparse coding
        ...
        % ZF detector
        r_zf{mixture_n, indd, f-1004, alg} = pinv(H)*wtz;

        % matched filter
        r_mf{mixture_n, indd, f-1004, alg} = H'*wtz;

        %neural networks
        r_nn{mixture_n, indd, f-1004, alg} = net(wtz);

        %logistic regression classifier
        pre_prob = mnrval(B, wtz');
        r_lr{mixture_n, indd, f-1004, alg} = pre_prob';
    end
